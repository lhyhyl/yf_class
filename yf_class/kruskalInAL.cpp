...
//	int** weights = (int **)malloc(sizeof(int*)*G->numV);//结果数组
//
//	for (int i = 0; i < G->numV;i++) {
...
//	}
//	int edges = 0;//
//	while (edges < G->numV - 1) {//循环条件
//		int weight = 32767;
//		int start = 0, end = 0;
//		for (int i = 0; i < G->numV;i++) {//遍历寻找最小边
//			for (EdgeNode* p = G->adjlist[i].firstEdge; p;p=p->next) {
//				if (weight>p->weight&&findAnster(fa,i)!=findAnster(fa,p->index)) {//更新条件
//					weight = p->weight;
//					start = i;
//					end = p->index;
//				}
//			}
//		}
//		weights[start][end] = weight;//当前边存入结果数组
//		weights[end][start] = weight;
//		unionn(fa,start,end);//合并
//		edges++;
//	}
//	outPut(G,weights);
//}
