...
//	while (!isEmpty(sq)) {
//		top(r);
//		for (EdgeNode* w = G->adjlist[r].firstEdge; w;w=w->next) {
//			if (!visited[w->index]) {
//				printf(w->index);
//				visited[w->index];
//				enqueue(w->index);
...
//	void DFS(ALGraph*, int*, VertexNode*);
//	int visited[10] = {0};//初试化
//	//int* visited = (int*)malloc(sizeof(int) * G->numV);//设置标记数组
//	for (int i = 0; i < G->numV; i++) {
//		visited[i] = 0;
...
//		if (w) {
//			if (!visited[w->index]) {
//				DFS(G,visited,&G->adjlist[w->index]);
//			}
//		}
//	}
//}
